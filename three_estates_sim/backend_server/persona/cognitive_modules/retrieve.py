...
def retrieve(persona, room, self_table_perceived, other_tables_perceived): 
    ...
    self_retrieved_lines_related = dict()
    ...
    for event in self_table_perceived: 
        ...
        if event.type == "chat":
            new_chat = new_retrieve(persona, [event.description], 5)
            # new_chat: {focal_point1: [list of nodes], focal_point2: [list of nodes], ...}
            # new_chat is a list of event nodes
            filtered_chat = filter_nonoverlapping_chat(new_chat, relevant_events, relevant_thoughts)
            self_retrieved_lines_related.update(filtered_chat)
    ...
    return retrieved_self, retrieved_others, self_retrieved_lines_related, other_retrieved_lines_related, retrieved_all_tables
...
def new_retrieve(persona, focal_points, n_count=30): 
  ...
  OUTPUT: 
    retrieved_self: A dictionary whose keys are a string focal point, and whose 
  ...
  """
  # <retrieved_self> is the main dictionary that we are returning
  retrieved_self = dict() 
...
#TODO if we even want to use this as a separate function
def new_retrieve_chat(persona, focal_points, n_count=30): 
  """
  Given the current persona and focal points (focal points are events or 
  thoughts for which we are retrieving), we retrieve a set of nodes for each
  of the focal points and return a dictionary. 
