...
class AssociativeMemory: 
  ...
  def save(self, out_json): 
    r = dict()
    persisted_nodes = sorted(self.seq_thought, key=lambda node: node.node_count, reverse=True)
    for save_count, node in enumerate(persisted_nodes, start=1):
      node_id = f"node_{save_count}"

      r[node_id] = dict()
      r[node_id]["node_count"] = save_count
    ...
    persisted_embedding_keys = {node.embedding_key for node in persisted_nodes}
    with open(out_json+"/embeddings.json", "w") as outfile:
      json.dump({
        key: self.embedding_to_json(value)
        for key, value in self.embeddings.items()
        if key in persisted_embedding_keys
      }, outfile)
